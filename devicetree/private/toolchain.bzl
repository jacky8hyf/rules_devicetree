...
def _devicetree_toolchain_impl(ctx):
    ...
    devicetree_toolchain_info = DevicetreeToolchainInfo(
        ...
        default_preprocessopts = ctx.attr.default_preprocessopts,
...
_common_attrs = {
    ...
    "default_preprocessopts": attr.string_list(doc = """Default list of flags to the C preprocessor.

        These are added after the flags that `dtb()` and `dtbo()` always pass
        (`-undef -D__DTS__` and the include directories from `deps`), and
        before `dtb(preprocessopts=)` and `dtbo(preprocessopts=)`.

        Setting this on a toolchain that does not preprocess is an error,
        because the flags would be silently dropped.
"""),
}
