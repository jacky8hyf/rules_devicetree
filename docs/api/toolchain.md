...
devicetree_toolchain(<a href="#devicetree_toolchain-name">name</a>, <a href="#devicetree_toolchain-default_dtcopts">default_dtcopts</a>, <a href="#devicetree_toolchain-default_preprocessopts">default_preprocessopts</a>, <a href="#devicetree_toolchain-dtc">dtc</a>, <a href="#devicetree_toolchain-fdtoverlay">fdtoverlay</a>, <a href="#devicetree_toolchain-preprocess">preprocess</a>)
</pre>
...
| <a id="devicetree_toolchain-default_preprocessopts"></a>default_preprocessopts |  Default list of flags to the C preprocessor.<br><br>These are added after the flags that `dtb()` and `dtbo()` always pass (`-undef -D__DTS__` and the include directories from `deps`), and before `dtb(preprocessopts=)` and `dtbo(preprocessopts=)`.<br><br>Setting this on a toolchain that does not preprocess is an error, because the flags would be silently dropped.   | List of strings | optional |  `[]`  |
